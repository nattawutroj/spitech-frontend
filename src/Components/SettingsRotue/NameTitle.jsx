...
const Example = () => {
    ...
    function fecthData() {
        Axios.get('/resources/admin/nametitle')
            .then(res => {
    ...
    const columns = useMemo(
        () => [
            ...
            {
                accessorKey: 'name_title_th',
                header: 'คำนำหน้าภาษาไทย',
                size: 100,
                muiEditTextFieldProps: {
                    type: 'name_title_th',
                    required: true,
            ...
            {
                accessorKey: 'name_title_en',
                header: 'คำนำหน้าภาษาอังกฤษ',
                size: 100,
                muiEditTextFieldProps: {
                    type: 'name_title_en',
                },
    ...
    const handleCreateUser = async ({ values, table }) => {
        console.log(values);
        if (values.name_title_th !== '' ) {
            Axios.post('/resources/admin/nametitle/add', values)
                .then(res => {
    ...
    const handleSaveUser = async ({ values, table }) => {
        console.log(values);
        Axios.put('/resources/admin/nametitle/edit', values)
            .then(res => {
                console.log(res.data);
                Axios.get('/resources/admin/nametitle')
                    .then(res => {
    ...
    const openDeleteConfirmModal = (row) => {
        ...
        if (window.confirm(`คุณต้องการลบข้อมูลนี้หรือไม่?`)) {
            Axios.delete('/resources/admin/nametitle/delete', { data: { id_name_title: row.id } })
                .then(res => {
                    console.log(res.data);
                    Axios.get('/resources/admin/nametitle')
                        .then(res => {
    ...
    const table = useMaterialReactTable({
        ...
        enableEditing: true,
        getRowId: (row) => row.id_name_title,
